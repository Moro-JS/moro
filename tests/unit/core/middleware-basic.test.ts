import { withLogging, withCaching, withRetry, withTimeout } from '../../../src/core/utilities/container';
...
describe('Container Higher-Order Functions', () => {
  ...
  describe('withLogging', () => {
    ...
    it('should handle factories with no dependencies', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'Creating service with dependencies: '
      );
    });
  ...
  describe('Higher-order function composition', () => {
    it('should compose multiple wrappers', async () => {
      ...
      const composedFactory = withTimeout(1000)(
        withRetry(2)(
          withLogging(mockLogger)(mockFactory)
        )
      );
    ...
    it('should handle composition with failures and retries', async () => {
      ...
      const composedFactory = withRetry(3)(
        withLogging(mockLogger)(mockFactory)
      );
