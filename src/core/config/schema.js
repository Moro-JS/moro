...
const ServerConfigSchema = zod_1.z.object({
    ...
    maxConnections: zod_1.z.coerce.number().min(1).default(1000).describe('Maximum concurrent connections'),
    timeout: zod_1.z.coerce.number().min(1000).default(30000).describe('Request timeout in milliseconds'),
});
...
const ServiceDiscoveryConfigSchema = zod_1.z.object({
    enabled: zod_1.z.coerce.boolean().default(false).describe('Enable service discovery'),
    type: zod_1.z
...
const DatabaseConfigSchema = zod_1.z.object({
    ...
    redis: zod_1.z.object({
        url: zod_1.z.string().default('redis://localhost:6379').describe('Redis connection URL'),
...
const ModuleDefaultsConfigSchema = zod_1.z.object({
    cache: zod_1.z.object({
        enabled: zod_1.z.coerce.boolean().default(true).describe('Enable caching by default'),
        defaultTtl: zod_1.z.coerce.number().min(0).default(300).describe('Default cache TTL in seconds'),
        maxSize: zod_1.z.coerce.number().min(1).default(1000).describe('Maximum cache entries'),
        strategy: zod_1.z.enum(['lru', 'lfu', 'fifo']).default('lru').describe('Cache eviction strategy'),
    }),
    rateLimit: zod_1.z.object({
        enabled: zod_1.z.coerce.boolean().default(true).describe('Enable rate limiting by default'),
        defaultRequests: zod_1.z.coerce.number().min(1).default(100).describe('Default requests per window'),
    ...
    validation: zod_1.z.object({
        enabled: zod_1.z.coerce.boolean().default(true).describe('Enable validation by default'),
        stripUnknown: zod_1.z.coerce
            .boolean()
            .default(true)
            .describe('Strip unknown properties from validated data'),
        abortEarly: zod_1.z.coerce.boolean().default(false).describe('Stop validation on first error'),
    }),
});
...
const LoggingConfigSchema = zod_1.z.object({
    ...
    format: zod_1.z.enum(['pretty', 'json', 'compact']).default('pretty').describe('Log output format'),
    enableColors: zod_1.z.coerce.boolean().default(true).describe('Enable colored log output'),
    enableTimestamp: zod_1.z.coerce.boolean().default(true).describe('Include timestamp in logs'),
    enableContext: zod_1.z.coerce.boolean().default(true).describe('Include context information in logs'),
...
const SecurityConfigSchema = zod_1.z.object({
    cors: zod_1.z.object({
        ...
        origin: zod_1.z.union([zod_1.z.string(), zod_1.z.array(zod_1.z.string()), zod_1.z.boolean()]).default('*'),
        methods: zod_1.z.array(zod_1.z.string()).default(['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS']),
